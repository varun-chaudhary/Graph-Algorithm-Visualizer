...
public class Board extends MouseAdapter implements ActionListener, ChangeListener {
    ...
    private JComboBox mazeGeneratorAlgosComboBox = new JComboBox(new String[] { // Dropdown menu of maze generator
                                                                                // algorithms
    ...
    public void setStartNode(Node startNode) {
        ...
        if (this.startNode == null) {
            ...
            this.rbStartNode.setText("Start node: (" + (startNode.getX() + 1) + ", " + (startNode.getY() + 1) + ")"); // Shows
                                                                                                                      // where
                                                                                                                      // start
                                                                                                                      // node
                                                                                                                      // is
    ...
    public void setEndNode(Node endNode) {
        ...
        if (endNode == null) {
            ...
            this.rbEndNode.setText("End node: (" + (endNode.getX() + 1) + ", " + (endNode.getY() + 1) + ")"); // Shows
                                                                                                              // where
                                                                                                              // end
                                                                                                              // node is
                                                                                                              // located.
    ...
    public void updateStartSearchButton() {
        if (getStartNode() != null && getEndNode() != null) { // If there are start/end nodes set, allow start button to
                                                              // be pressed
        ...
        if (evt.getSource() == this.timerBoard) { // 60FPS Timer
            ...
            if (this.pathfindingOngoing == true) { // is ongoing
                ...
                new Thread(() -> { // Thread to wait for connect path thread to end
                    if (this.threadPathNodeRefresh != null) { // If connectPath thread is running, wait for it to die
                                                              // before executing
                        try {
            ...
            new Thread(() -> {
                if (JOptionPane.showConfirmDialog(null,
                        "To generate a maze the current grid will be reset. Are you sure you want to clear the grid?",
                        "Warning",
                        JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) { // Confirm clear grid dialog. Yes
                                                                                // option.
                    clearGrid();
    ...
    public void mouseDrawing(MouseEvent evt) { // Method for draw tools (aggregates mousePressed & mouseDragged)
        ...
        if (mouseX >= Main.MENU_WIDTH && mouseX < Main.FRAME_WIDTH && mouseY >= 0 && mouseY < Main.FRAME_HEIGHT
                && this.grid != null) { // Mouse pointer is within grid (not in menu bar)
            ...
            if (this.disableDraw == false) { // Drawing is enabled
                if (SwingUtilities.isLeftMouseButton(evt) && this.grid[gridY][gridX].isEmpty()) { // Left click (draw)
                    if (this.rbStartNode.isSelected() && this.startNode == null) { // Checks which draw tool is
                                                                                   // selected. Also limits only one
                                                                                   // start/end node
    ...
    public Board() {
        ...
        this.labelDrawingTools.setBounds(Main.MENU_PADDING, Main.MENU_PADDING + 80,
                Main.MENU_WIDTH - (Main.MENU_PADDING * 2), 30);
        ...
        this.butResetGrid.setBounds(Main.MENU_PADDING, Main.MENU_PADDING + 90 + 10 + (20 * 5),
                (Main.MENU_WIDTH / 2) - Main.MENU_PADDING, 40);
        this.butResetGrid.setFocusable(false);
        this.butResetGrid.addActionListener(this);

        // Clear grid button
        this.boardPanel.add(this.butClearGrid);
        this.butClearGrid.setBounds(this.butResetGrid.getWidth() + Main.MENU_PADDING,
                Main.MENU_PADDING + 90 + 10 + (20 * 5), (Main.MENU_WIDTH / 2) - Main.MENU_PADDING, 40);
        ...
        this.labelGridWidth.setBounds(Main.MENU_PADDING, Main.MENU_PADDING + 90 + (20 * 8),
                Main.MENU_WIDTH - (Main.MENU_PADDING * 2), 30);

        // Grid width slider
        this.boardPanel.add(this.sliderGridWidth);
        this.sliderGridWidth.setBounds(Main.MENU_PADDING, Main.MENU_PADDING + 90 + 5 + (20 * 9),
                Main.MENU_WIDTH - (Main.MENU_PADDING * 2), 50);
         ...
         * this.labelMazeGeneratorAlgos.setBounds(Main.MENU_PADDING,420,
         * Main.MENU_WIDTH-(Main.MENU_PADDING*2), 30);
         * 
         * // Maze Generator Algorithms Dropdown (ComboBox)
         * this.boardPanel.add(this.mazeGeneratorAlgosComboBox);
         * this.mazeGeneratorAlgosComboBox.setBounds(Main.MENU_PADDING, 420+30,
         * Main.MENU_WIDTH-(Main.MENU_PADDING*2)-80-5,30);
         ...
         * this.butGenerateMaze.setBounds(Main.MENU_WIDTH-(Main.MENU_PADDING)-80,420+30-
         * 1,80,32);
         * this.butGenerateMaze.setFocusable(false);
         * this.butGenerateMaze.addActionListener(this);
         * this.butGenerateMaze.setEnabled(false); // When maze generator algo is none,
         * set to off. (Default to off)
         */
        ...
        this.sliderPathRefreshInterval.setBounds(Main.MENU_PADDING, 490 + 25, Main.MENU_WIDTH - (Main.MENU_PADDING * 2),
                50);
        ...
        tablePathRefreshInterval.put(
                (this.sliderPathRefreshInterval.getMaximum() + this.sliderPathRefreshInterval.getMinimum()) / 2,
                new JLabel("Medium"));
...
class BoardPanel extends JPanel {
    ...
    public void paintComponent(Graphics graphic) {
        ...
        if (pathfindingComplete == true) { // Modified RGB Color fade algorithm from
        ...
        if (this.endNotFound == true) {
            ...
            g2.drawString(noPathFound, Main.MENU_WIDTH + (Main.FRAME_HEIGHT / 2) - (stringWidth / 2),
                    (Main.FRAME_HEIGHT / 2));
        }
    }
        ...
        for (int i = 0; i < grid.length; i++) {
            for (int j = 0; j < grid[i].length; j++) {
                ...
                if (grid[i][j].isStart() || grid[i][j].isEnd()) { // Draw border around start/end nodes
                    g2.setStroke(new BasicStroke(2));
                    g2.setColor(Color.BLACK);
                    g2.drawRect(Main.MENU_WIDTH + (j * sideLength) + 1, (i * sideLength) + 1, sideLength - 1,
                            sideLength - 1); // Draw node
                    g2.setStroke(new BasicStroke(1));
                }
            }
        }
